...
function fetchUserDetails($username=NULL,$token=NULL, $id=NULL)
{
	...
	$stmt->execute();
	$stmt->bind_result($id, $user, $display, $password, $email, $token, $activationRequest, $passwordRequest, $active, $title, $signUp, $signIn, $experiment_ok);
	while ($stmt->fetch()){
		$row = array('id' => $id, 'user_name' => $user, 'display_name' => $display, 'password' => $password, 'email' => $email, 'activation_token' => $token, 'last_activation_request' => $activationRequest, 'lost_password_request' => $passwordRequest, 'active' => $active, 'title' => $title, 'sign_up_stamp' => $signUp, 'last_sign_in_stamp' => $signIn, 'experiment_ok' => $experiment_ok);
	}
	$stmt->close();
	return ($row);
}
...
function getFreeTimeSlots(){
	...
	$stmt = $mysqli->prepare("SELECT id, timestamp FROM reservation_slots WHERE free = 1");
	$stmt->execute();
	$stmt->bind_result($id, $timestamp);
	while ($stmt->fetch()){
		$row[$id] = array('id' => $id, 'timestamp' => $timestamp);
	}
	$stmt->close();
	return ($row);
}
...
function getTimeSlotById($id){
	global $mysqli;
	$stmt = $mysqli->prepare("SELECT timestamp FROM reservation_slots WHERE id = ? LIMIT 1");
	$stmt->bind_param("i", $id);
	$stmt->execute();
	$stmt->bind_result($t);
	$stmt->fetch();
...
function setTimeSlotToUser($id, $username) {
	global $mysqli;

	$stmt = $mysqli->prepare("UPDATE reservation_slots SET free=0, username=? WHERE id = ?");
...
function setUserExperimentOk($username) {
	global $mysqli,$db_table_prefix;
	$stmt = $mysqli->prepare("UPDATE ".$db_table_prefix."users
		SET experiment_ok = 1
		WHERE
...
function fetchAllApplicationProfiles()
{
	global $mysqli;
	$stmt = $mysqli->prepare("SELECT id, Name, createdBy, type, timestamp FROM application_profiles");
	$stmt->execute();
	$stmt->bind_result($id, $name, $createdBy, $type, $timestamp);
	
	while ($stmt->fetch()){
		$row[] = array('id' => $id, 'name' => $name, 'createdBy' => $createdBy, 'type' => $type, 'timestamp' => $timestamp);
	}
	$stmt->close();
...
function fetchApplicationProfileById($id)
{
	global $mysqli;
	$stmt = $mysqli->prepare("SELECT id, Name, type, PA_min, PA_max, PA_weight,
							 RA_min, RA_max, RA_weight, 
							 LA_min, LA_max, LA_weight, 
							 EN_min, EN_max, EN_weight, 
							 PA_type, LA_type, EN_type FROM application_profiles WHERE id = ? LIMIT 1");
	$stmt->bind_param("i", $id);
	$stmt->execute();
	$stmt->bind_result($id, $name, $type,	$PA_min, $PA_max, $PA_weight,
						$RA_min, $RA_max, $RA_weight,
						$LA_min, $LA_max, $LA_weight,
						$EN_min, $EN_max, $EN_weight,
	...
	$stmt->fetch();
	$stmt->close();
	$app = array('id' => $id, 'name' => $name, 'type' => $type,	'PA_min' => floatval($PA_min), 'PA_max' => floatval($PA_max), 'PA_weight' => floatval($PA_weight),
									'RA_min' => floatval($RA_min), 'RA_max' => floatval($RA_max), 'RA_weight' => floatval($RA_weight),
									'LA_min' => floatval($LA_min), 'LA_max' => floatval($LA_max), 'LA_weight' => floatval($LA_weight),
									'EN_min' => floatval($EN_min), 'EN_max' => floatval($EN_max), 'EN_weight' => floatval($EN_weight),
									'PA_type' => $PA_type, 'LA_type' => $LA_type, 'EN_type' => $EN_type);
...
{
	global $mysqli;
	$stmt = $mysqli->prepare("SELECT testbed, approach, interference, currentTime FROM uc_experiments WHERE demo = 1");
	$stmt->execute();
	$stmt->bind_result($testbed, $approach, $interference, $timestamp);
	
	while ($stmt->fetch()){
		$row[] = array('testbed' => $testbed, 'approach' => $approach, 'interference' => $interference, 'timestamp' => $timestamp);
	}
	$stmt->close();
...
function getExperimentByName($name){
	global $mysqli;
	$stmt = $mysqli->prepare("SELECT timeslot, testbed, approach, channel, txpower, interference, path FROM uc_experiments WHERE username = ? LIMIT 1");
	$stmt->bind_param("s", $name);
	$stmt->execute();
	$stmt->bind_result($timeslot, $testbed, $approach, $channel, $txpower, $interference, $path);
	$stmt->fetch();
	$stmt->close();

	$stmt = $mysqli->prepare("SELECT id FROM reservation_slots WHERE timestamp = ? LIMIT 1");
	...
	$exp = array('timeslot' => $timeslot, 'timeslotId' => strval($id), 'testbed' => $testbed, 'approach' => $approach, 'channel' => $channel, 'txpower' => $txpower, 'interference' => $interference, 'path' => $path);
	return ($exp);
}

function updateExperimentToDB($username, $testbed, $approach, $channel, $txpower, $interference, $robot)
{
	global $mysqli,$db_table_prefix;

	//Insert the user into the database providing no errors have been found.
	$stmt = $mysqli->prepare("UPDATE ".$db_table_prefix."experiments SET
		testbed = ?, approach = ?, channel = ?, txpower = ?, interference = ?, path = ? WHERE username = ?");

	$stmt->bind_param("sssssss", $testbed, $approach, $channel, $txpower, $interference, $robot, $username);
	$stmt->execute();
	$stmt->close();
...
function demoStarted($username, $time)
{
	global $mysqli,$db_table_prefix;

	//Insert the user into the database providing no errors have been found.
	$stmt = $mysqli->prepare("UPDATE ".$db_table_prefix."experiments SET currentTime = ?, demo = 1 WHERE username = ?");
